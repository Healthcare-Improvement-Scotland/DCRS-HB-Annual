...
RunChart = function(measure, subgroup, exclude, shiftsens, percentage) {
  
  dataDF = data.frame(measure,subgroup, exclude)%>%
  ...
  if (any(is.na(measure) | exclude == 1)) {
    
    missingDF = data.frame(measure,subgroup, exclude)%>%
    filter(is.na(measure) | exclude == 1)%>%
     #filter(is.na(measure))%>%
      mutate(median = NA,
             baselines = NA,
             base_n = NA,
  ...
  dataDF$median = median(dataDF$measure[shiftpos:(shiftpos+7)])
  ...
  while (i <= length(dataDF$measure)) {
    ...
    if(shiftsens == "none") {
      newshiftpos = min(which.max(dataDF$runlength >= 9 & dataDF$runNo >= dataDF$runNo[shiftpos+20] & index(dataDF$runNo) > shiftpos + 7),nrow(dataDF))}
    ...
    if (shiftpos+7 <= length(dataDF$measure) & (shiftpos+7 < newshiftpos | newshiftpos == 1))
    {dataDF$baselines[shiftpos:(shiftpos+7)] = dataDF$median[shiftpos:(shiftpos+7)]
    # Increment base_n to disconnect baselines
    base_n <- base_n + 1
    dataDF$base_n[shiftpos:length(dataDF$base_n)] = base_n
    # Label baseline
    dataDF$base_label[shiftpos] = paste0("Median ", dataDF$base_n[shiftpos], " = ",
                                         percent(dataDF$median[shiftpos], accuracy = 0.1))
    }
    
    # If rebase occurs over 9-11 points at end of dataframe, rebase as temporary
    else if (shiftpos+7 > length(dataDF$measure))
    {d <- length(dataDF$measure) - shiftpos
    # Increment base_n to disconnect baselines
    base_n <- base_n + 1
    dataDF$base_n[shiftpos:length(dataDF$base_n)] = base_n
    # Label baseline (Temporary)
    dataDF$base_label[shiftpos] = paste0("Temporary: ", signif(dataDF$median[shiftpos],2))
    break
    ...
    dataDF$median[newshiftpos:length(dataDF$measure)] = median(dataDF$measure[newshiftpos:min(newshiftpos+7,(newshiftpos+dataDF$runlength[newshiftpos]-1), nrow(dataDF))])  # Median
 ...
 # if (min(dataDF$base_n) > max(dataDF$base_n)) {
  #  dataDF$change_label[shiftpos] = paste0("Reduction from baseline: ", percent(1-(max(dataDF$median[shiftpos])/min(dataDF$median[shiftpos])), 
   #                                                                             accuracy = 0.1))
#  } else {
 #   "none"
#  }   
